...
*   **Поиск**: Клиентский поиск по названию документа, названию раздела и содержимому README с подсветкой совпадений.
*   **Якоря заголовков**: Для всех заголовков второго уровня (`h2`) в README автоматически генерируются `id`-атрибуты для прямой ссылки на раздел.
...
org_name: "Мурманская таможня" # название организации в шапке страницы
```
...
Поиск выполняется полностью на стороне браузера и не нагружает сервер. Поддерживаются:

* Поиск по названию документа, названию раздела и содержимому README (нормализуется регистр и `ё` → `е`).
* Подсветка совпадений в списке документов и в тексте README.
* Синхронизация запроса с адресной строкой: при вводе запроса он попадает в `?q=...`, а при открытии страницы по такой ссылке фильтр применяется автоматически.
* Операторы:
  * `sec:текст` — искать только по названиям разделов.
  * `doc:текст` — искать только по именам файлов.
  * `readme:текст` — искать только по содержимому описаний разделов (README).

Примеры:

* `приказ` — найти все документы, разделы и описания, содержащие это слово.
* `sec:кадровый` — отфильтровать только разделы, в названии которых есть «кадровый».
* `doc:отчет` — показать только документы с «отчет» в названии.
* `readme:инструкция` — найти разделы, в README которых встречается слово «инструкция».

## Якоря заголовков

При рендеринге `README.md` сервер автоматически добавляет `id`-атрибут каждому заголовку второго уровня (`## Заголовок`). Это позволяет ссылаться на конкретный раздел страницы.

Формат `id`: префикс `h2-` + текст заголовка, приведённый к нижнему регистру, пробелы заменены на дефисы, удалено всё кроме букв, цифр, дефисов и подчёркиваний.

**Пример:**

```markdown
## 1. инструкция по ОТ при работе с ПК (оргтехникой)
```

→ `<h2 id="h2-1-инструкция-по-от-при-работе-с-пк-оргтехникой">`

Ссылка на этот раздел: `#h2-1-инструкция-по-от-при-работе-с-пк-оргтехникой`

**В оглавлении README** можно использовать такие ссылки:

```markdown
1. [Инструкция по ОТ при работе с ПК](#h2-1-инструкция-по-от-при-работе-с-пк-оргтехникой)
```
